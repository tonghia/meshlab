...
void DecorateBasePlugin::decorateMesh(const QAction* a, MeshModel &m, const RichParameterList *rm, GLArea *gla, QPainter *painter, GLLogStream &_log)
{
	...
	case DP_SHOW_VERT_QUALITY_HISTOGRAM :
	{
		...
		qH = vcg::tri::Allocator<CMeshO>::GetPerMeshAttribute<CHist>(m.cm,"VertexQualityHist");
		CHist &ch=qH();
...
bool DecorateBasePlugin::isDecorationApplicable(const QAction* action, const MeshModel& m, QString &ErrorMessage) const
{
	...
	if (ID(action) == DP_SHOW_VERT_QUALITY_HISTOGRAM)
	{
		...
		{
			ErrorMessage = QString("<br>CANNOT START DECORATOR: the layer does not contain vertex quality");
			return false;
		}
	}
	if (ID(action) == DP_SHOW_FACE_QUALITY_HISTOGRAM)
	{
		...
		{
			ErrorMessage = QString("<br>CANNOT START DECORATOR: the layer does not contain face quality");
			return false;
		}
	}
...
bool DecorateBasePlugin::startDecorate(const QAction * action, MeshModel &m, const RichParameterList *rm, GLArea *gla)
{
	...
	case DP_SHOW_VERT_QUALITY_HISTOGRAM :
	{
		...
		CMeshO::PerMeshAttributeHandle<CHist > qH = vcg::tri::Allocator<CMeshO>::GetPerMeshAttribute<CHist>(m.cm,"VertexQualityHist");
...
void DecorateBasePlugin::initGlobalParameterList(const QAction* action, RichParameterList &parset)
{
	...
	case DP_SHOW_VERT_QUALITY_HISTOGRAM :
	{
		...
		parset.addParam(RichBool(perVertexHistAreaParam(), false,"Area Weighted","If true the histogram is computed according to the surface of the involved elements.<br>"
	...
	case DP_SHOW_FACE_QUALITY_HISTOGRAM :
	{
		...
		parset.addParam(RichBool(perFaceHistAreaParam(), false,"Area Weighted","If true the histogram is computed according to the surface of the involved elements.<br>"
			"e.g. each face contribute to the histogram proportionally to its area and each vertex with 1/3 of sum of the areas of the incident triangles."));
